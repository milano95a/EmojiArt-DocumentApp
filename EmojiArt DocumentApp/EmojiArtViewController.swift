...
//  EmojiArt_UIKit
//
//  Created by Abdurakhmon Jamoliddinov on 12/14/20.
//
...
class EmojiArtViewController: UIViewController, UIDropInteractionDelegate, UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDragDelegate, UICollectionViewDropDelegate, UIPopoverPresentationControllerDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "ShowDocumentInfo" {
            if let destination = segue.destination.contents as? DocumentInfoViewController {
                document?.thumbnail = artView.snapshot
    ...
    var emojiArt: EmojiArt? {
        get {
            if let url = emojiArtBackgroundImage.url {
                let emojis = artView.subviews.flatMap { $0 as? UILabel}.flatMap{ EmojiArt.EmojiInfo(label: $0) }
                ...
                imageFetcher = ImageFetcher(fetch: url){ (url, image) in
                    DispatchQueue.main.async { [weak self] in
                        ...
                        newValue?.emojis.forEach{
                            ...
                            self?.artView.addLabel(with: attributedText, centeredAt: CGPoint(x: $0.x, y: $0.y))
                        }
                    }
                }
            }
    ...
    var emojiArtBackgroundImage: (url: URL?, image: UIImage?) {
        get {
            return (_emojiArtBackgroundImageURL, artView.backgroundImage)
        }
        set {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if indexPath.section == 1 {
            ...
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "AddEmojiButton", for: indexPath)
            return cell
        }
    }
            ...
            if let sourceIndexPath = item.sourceIndexPath {
                if let attributedString = item.dragItem.localObject as? NSAttributedString {
                    collectionView.performBatchUpdates({
                        ...
                        emojiCollectionView.insertItems(at: [destinationIndexPath])
                        emojiCollectionView.deleteItems(at: [sourceIndexPath])
                    })
    ...
    private func presentBadURLWarning(for url: URL){
        if  suppressBadURLWarnings { return }
        ...
        let action = UIAlertAction(title: "Keep Warning", style: .default) { action in
            self.suppressBadURLWarnings = false
        }
        
        let action2 = UIAlertAction(title: "Stop Warning", style: .destructive) { action in
        ...
        session.loadObjects(ofClass: NSURL.self) { nsurls in
            if let url = nsurls.first as? URL {
                ...
                DispatchQueue.global(qos: .userInitiated).async {
                    if let downloadedImage = try? Data(contentsOf: url), let image = UIImage(data: downloadedImage){
                        DispatchQueue.main.async {
                            self.emojiArtBackgroundImage = (url, image)
                        }
                    } else {
